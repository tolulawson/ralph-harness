...
10. Do not write project-specific contract or control-plane instructions into `.ralph/runtime-contract.md` or Ralph-managed runtime skill directories; write them into `.ralph/policy/runtime-overrides.md`, `.ralph/policy/project-policy.md`, and `.ralph/context/project-facts.json` extension fields.
11. Install the repo-local hook surfaces for Codex, Claude Code, and Cursor plus the shared `.ralph/hooks/stop-boundary.py` file.
12. Use the canonical `AGENTS.md` and `CLAUDE.md` loader guidance plus the install checklist from `INSTALLATION.md` rather than inventing or rephrasing the install flow.
13. Complete the installation without copying any repo-root source files outside `src/` from the source repository.
