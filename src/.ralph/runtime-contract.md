...
- The main thread must never continue as the PRD or planning coordinator after launcher handoff begins. Queue-wide control-plane coordination belongs only to orchestrator peers operating through the shared scheduler lock, and adapters that cannot spawn the required `ralph-prd` or `ralph-plan` subagent are unsupported rather than allowed to fall back to inline execution.
...
- No delegated worker role besides the active Ralph coordinator may mutate shared queue state, workflow state, scheduler-lock state, projections, promoted learnings, or event logs.
