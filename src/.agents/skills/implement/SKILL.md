...
- active claim from `.ralph/state/worker-claims.json`
...
3. Confirm the assigned worktree path matches the active spec branch and inspect that spec worktree state before editing.
...
14. Do not append orchestrator events or mutate shared queue, workflow state, lease state, or task lifecycle state directly.
...
19. Write the implementation report.
