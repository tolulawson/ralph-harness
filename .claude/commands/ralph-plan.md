...
Keep the invoking thread thin: launch a dedicated Ralph planning coordinator subagent immediately, let that coordinator sequence `specify`, same-batch `research`, `plan`, `task-gen`, and `plan-check` as needed, then wait and relay its result instead of planning inline on the command thread.
