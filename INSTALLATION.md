...
- [src/install-manifest.txt](https://github.com/tolulawson/ralph-harness/blob/v0.2.0/src/install-manifest.txt)

The canonical generated-runtime contract is:

- [src/generated-runtime-manifest.txt](https://github.com/tolulawson/ralph-harness/blob/v0.2.0/src/generated-runtime-manifest.txt)
...
1. [INSTALLATION.md](https://github.com/tolulawson/ralph-harness/blob/v0.2.0/INSTALLATION.md)
2. [src/install-manifest.txt](https://github.com/tolulawson/ralph-harness/blob/v0.2.0/src/install-manifest.txt)
3. [src/generated-runtime-manifest.txt](https://github.com/tolulawson/ralph-harness/blob/v0.2.0/src/generated-runtime-manifest.txt)
4. [VERSION](https://github.com/tolulawson/ralph-harness/blob/v0.2.0/VERSION)
...
Use tag v0.2.0 from that repository unless the user explicitly requests another release.
...
Adapt the constitution, runtime contract pointers, project policy, and copied knowledge files for this repo, preserve existing code, create the project PRD, decompose it into epochs and numbered specs, seed the initial FIFO spec queue, and update .ralph/harness-version.json with tag v0.2.0 and the resolved commit.
```
...
Then create the runtime files listed in [src/generated-runtime-manifest.txt](https://github.com/tolulawson/ralph-harness/blob/v0.2.0/src/generated-runtime-manifest.txt), refresh the managed Ralph block in `AGENTS.md`, update `.ralph/harness-version.json`, and adapt the target repository before first use.
...
After copying the scaffold, create the runtime records listed in [src/generated-runtime-manifest.txt](https://github.com/tolulawson/ralph-harness/blob/v0.2.0/src/generated-runtime-manifest.txt):
...
7. update `.ralph/harness-version.json` with tag `v0.2.0` and the resolved source commit
...
6. records tag `v0.2.0` plus the resolved source commit in `.ralph/harness-version.json`
...
- `.ralph/harness-version.json` exists, parses, and records tag `v0.2.0` plus the resolved source commit
